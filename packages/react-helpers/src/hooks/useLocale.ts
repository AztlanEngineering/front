...
export default function useLocale(
  ...
  loadLocaleData: (string) => Promise<any>,
  ...
  const getBrowserLocale = useCallback(() => {
    if (isClient && navigator.language) {
      return navigator.language.slice(0, 2) // Extracts the two-letter locale code
    }
  ...
  }, [getBrowserLocale, initialLocale, storageKey])
